...
fileprivate extension PullingHeader {
    ...
    func updateHeaderFrame() {
        ...
        case .down:
            ...
            pullToRefreshView.frame = CGRect(origin: CGPoint(x: 0, y: originY),
                                            size: CGSize(width: scrollView.up2p.width, height: refreshViewDirectionLength))
        case .left:
            let originX = scrollView.contentSize.width + scrollViewDirectionInset
            pullToRefreshView.frame = .init(origin: CGPoint(x: originX, y: 0),
                                            size: CGSize(width: refreshViewDirectionLength, height: scrollView.up2p.height))
        }
    }
